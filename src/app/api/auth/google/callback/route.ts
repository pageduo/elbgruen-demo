...
export async function GET(request: NextRequest) {
  ...
  const expectedState = request.cookies.get("elbgruen_oauth_state")?.value;
  const redirectTo = request.cookies.get("elbgruen_oauth_redirect")?.value ?? "/";
  ...
  response.cookies.delete("elbgruen_oauth_state");
  response.cookies.delete("elbgruen_oauth_redirect");
  return response;
}
